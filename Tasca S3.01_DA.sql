...
ALTER TABLE transaction MODIFY credit_card_id VARCHAR(15) DEFAULT NULL;
...
ALTER TABLE credit_card MODIFY iban VARCHAR(34) DEFAULT NULL;
ALTER TABLE credit_card MODIFY pan VARCHAR(25) DEFAULT NULL;
ALTER TABLE credit_card MODIFY pin VARCHAR(4) DEFAULT NULL;
ALTER TABLE credit_card MODIFY cvv VARCHAR(5) DEFAULT NULL;
ALTER TABLE credit_card MODIFY expiring_date VARCHAR(65) DEFAULT NULL;
...
CREATE VIEW Vista_Media_Ventas AS
...
SELECT * FROM Vista_Media_Ventas;

CREATE VIEW VistaMarketing AS
SELECT c.company_name, c.phone, c.country, vm.MediaVentas
FROM company c
INNER JOIN Vista_Media_Ventas vm ON c.company_name = vm.company_name;

SELECT * FROM VistaMarketing;

-- OTRA FORMA
CREATE VIEW VistaMarketingII AS
SELECT c.company_name, c.phone, c.country, Medias.MediaVentas
FROM company c
INNER JOIN (SELECT c.company_name, round(AVG(t.amount),2) AS MediaVentas
			FROM company c
			INNER JOIN transaction t ON c.id = t.company_id
			GROUP BY c.company_name
			ORDER BY MediaVentas DESC) AS Medias
ON c.company_name = Medias.company_name;

SELECT * FROM VistaMarketingII;
...
INSERT INTO user (id, name, surname, phone, email, birth_date, country, city, postal_code, address) VALUES (        "206", "Harper", "Hart", "030-656-1670", "fringilla.donec@outlook.net", "Nov 17, 2000",         "Canada", "Qu̩bec City", "B4K 0J6", "8588 Massa. Ave");
...
-- CREAR NUEVA TABLA DATA_USER
-- con DATOS DE UNA  subquery de USUARIOS DE LA TABLA USER QUE ESTAN EN LA TABLA TRANSACTION
CREATE TABLE data_user AS
SELECT *
FROM user u
WHERE u.id = any (SELECT DISTINCT u.id
					FROM user u
...
-- agregamos el constraint de primary key
ALTER TABLE data_user ADD CONSTRAINT PRIMARY KEY(id);
DESC data_user;
...
-- Creamos la relacion entre la tabla data_user y transaction de uno a muchos
ALTER TABLE transaction
